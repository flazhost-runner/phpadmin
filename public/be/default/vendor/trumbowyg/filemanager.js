/*!
 * Trumbowyg File Manager — plugin custom PHPAdmin (port dari NodeAdmin).
 * Tombol toolbar "filemanager" membuka modal: daftar gambar dari storage
 * (GET /admin/v1/media/list), upload baru (POST .../upload), hapus
 * (POST .../delete), dan sisip <img> ke editor saat thumbnail diklik.
 *
 * CSRF: token diambil dari input[name="_csrf"] atau window._csrf dan
 * dikirim via header x-csrf-token.
 *
 * Memerlukan jQuery + Trumbowyg (sudah dimuat di admin_head.php).
 * Modal pakai markup + style sendiri (tanpa Bootstrap JS).
 */
(function ($) {
    'use strict';
    if (!$ || !$.trumbowyg) return;
    ...
    function csrfToken() {
        ...
        return (typeof window._csrf !== 'undefined') ? window._csrf : '';
    }

    // --- Modal (di-inject sekali ke body) ---
    function ensureModal() {
        ...
        var css =
            ...
            '.tb-fm-btn-primary{background:var(--primary,#2563eb);color:#fff;border:0;border-radius:6px;padding:6px 16px;cursor:pointer;white-space:nowrap}' +
            ...
            '.tb-fm-thumb:hover{outline:2px solid var(--primary,#2563eb)}' +
    ...
    function bindEvents() {
        ...
        // Hapus file — kirim JSON body (PHPAdmin MediaController baca php://input)
