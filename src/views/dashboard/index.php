 ...
 *   $stats      array{users:int,roles:int,permissions:int}
 *   $activities list<array{icon,bg,iconColor,title,subtitle}>
...
/** @var list<array{icon:string,bg:string,iconColor:string,title:string,subtitle:string}> $activities */
/** @var array{primary:string,secondary:string,light:string,dark:string} $theme */
/** @var string $themeName */
/** @var \PHPAdmin\Modules\Access\Models\User|null $currentUser */
?>
  ...
  <div class="stat-card bg-white rounded-xl shadow-lg p-6 hover-scale" style="border-left:4px solid var(--primary)">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-3xl font-bold text-gray-800 counter" data-target="<?= (int)($stats['users'] ?? 0) ?>">0</p>
  ...
  <div class="stat-card bg-white rounded-xl shadow-lg p-6 hover-scale border-l-4 border-green-500">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-3xl font-bold text-gray-800 counter" data-target="<?= (int)($stats['roles'] ?? 0) ?>">0</p>
  ...
  <div class="stat-card bg-white rounded-xl shadow-lg p-6 hover-scale border-l-4 border-yellow-500">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-3xl font-bold text-gray-800 counter" data-target="<?= (int)($stats['permissions'] ?? 0) ?>">0</p>
  ...
  <div class="stat-card bg-white rounded-xl shadow-lg p-6 hover-scale border-l-4 border-purple-500">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-2xl font-bold text-gray-800"><?= e($themeName) ?></p>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
  ...
  <div class="bg-white rounded-xl shadow-lg p-6">
    ...
    <?php
    $topProducts = [
      ['ic' => 'fa-headphones', 'c' => 'var(--primary)', 'n' => 'Premium Headphones', 'cat' => 'Electronics • 234 sold', 'p' => '$299', 'd' => '+15%', 'up' => true, 'rank' => 1],
      ['ic' => 'fa-clock',      'c' => '#10B981',         'n' => 'Smart Watch',        'cat' => 'Wearables • 189 sold',   'p' => '$199', 'd' => '+8%',  'up' => true, 'rank' => 2],
      ['ic' => 'fa-laptop',     'c' => '#F59E0B',         'n' => 'Laptop Stand',       'cat' => 'Accessories • 156 sold', 'p' => '$49',  'd' => '-3%',  'up' => false,'rank' => 3],
      ['ic' => 'fa-mouse',      'c' => '#8B5CF6',         'n' => 'Gaming Mouse',       'cat' => 'Gaming • 143 sold',      'p' => '$79',  'd' => '+12%', 'up' => true, 'rank' => 4],
      ['ic' => 'fa-mobile-alt', 'c' => '#EF4444',         'n' => 'Phone Case',         'cat' => 'Accessories • 127 sold', 'p' => '$25',  'd' => '+7%',  'up' => true, 'rank' => 5],
    ];
    ?>
    <div class="space-y-3">
      ...
      <div class="flex items-center p-4 border border-gray-100 rounded-xl hover:bg-gray-50 hover:shadow-md transition-all group">
        <div class="flex items-center w-4/5">
          <div class="relative flex-shrink-0 mr-3">
            <div class="w-10 h-10 rounded-lg shadow-sm flex items-center justify-center" style="background:<?= e($x['c']) ?>">
              <i class="fas <?= e($x['ic']) ?> text-white text-sm"></i>
            </div>
            <div class="absolute -top-1 -right-1 w-4 h-4 rounded-full flex items-center justify-center" style="background:<?= e($x['c']) ?>">
              <span class="text-white text-xs font-bold leading-none"><?= (int)$x['rank'] ?></span>
            </div>
          </div>
          <div class="flex-1 min-w-0">
            <h4 class="font-semibold text-gray-800"><?= e($x['n']) ?></h4>
            <p class="text-sm text-gray-500"><?= e($x['cat']) ?></p>
          </div>
        </div>
        <div class="text-right w-1/5 pl-3">
          <p class="text-lg font-bold text-gray-800"><?= e($x['p']) ?></p>
          <p class="text-sm flex items-center justify-end <?= $x['up'] ? 'text-green-600' : 'text-red-600' ?>">
            <i class="fas fa-arrow-<?= $x['up'] ? 'up' : 'down' ?> text-xs mr-1"></i><?= e($x['d']) ?>
          </p>
        </div>
      </div>
      <?php endforeach; ?>
    ...
    <div class="mt-6 pt-4 border-t border-gray-100">
      <button class="w-full text-center font-medium text-sm py-2 rounded-lg" style="color:var(--primary)">
  ...
  <div class="flex items-center justify-between mb-4">
    ...
    <div class="flex items-center gap-2">
      <div id="bulkActions" class="hidden items-center gap-2 mr-4">
        ...
        <button onclick="deleteSelectedOrders()" class="bg-red-500 hover:bg-red-600 text-white px-3 py-2 rounded-lg text-sm font-medium">
          <i class="fas fa-trash mr-1"></i>Delete
        </button>
        <button onclick="exportSelectedOrders()" class="bg-green-500 hover:bg-green-600 text-white px-3 py-2 rounded-lg text-sm font-medium">
          <i class="fas fa-file-export mr-1"></i>Export Selected
        </button>
      </div>
      <button class="text-white px-4 py-2 rounded-lg text-sm font-medium" style="background:var(--primary)">
  ...
  <div class="overflow-x-auto">
    <table class="w-full">
      ...
      <tbody>
        <?php
        $orders = [
          ['no' => 1, 'id' => '#12345', 'cust' => 'John Doe',     'bg' => '3B82F6', 'prod' => 'Premium Headphones', 'amt' => '$299.99', 'st' => 'Delivered',  'stc' => 'bg-green-100 text-green-800',  'date' => '2024-01-15'],
        ...
        <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
          ...
          <td class="py-3 px-4">
            <div class="flex items-center gap-2">
              <img src="https://ui-avatars.com/api/?name=<?= urlencode($o['cust']) ?>&background=<?= e($o['bg']) ?>&color=fff"
                   class="w-8 h-8 rounded-full" alt="<?= e($o['cust']) ?>">
              <span><?= e($o['cust']) ?></span>
            </div>
          </td>
          <td class="py-3 px-4"><?= e($o['prod']) ?></td>
          <td class="py-3 px-4 font-semibold"><?= e($o['amt']) ?></td>
          <td class="py-3 px-4">
            <span class="px-2 py-1 <?= e($o['stc']) ?> rounded-full text-sm"><?= e($o['st']) ?></span>
          </td>
          <td class="py-3 px-4 text-gray-600"><?= e($o['date']) ?></td>
          <td class="py-3 px-4">
...
<script>
  ...
  document.querySelectorAll('.counter').forEach(function (el) {
    ...
    var dur = 1500, step = target / (dur / 16), cur = 0;
  ...
  // Sales line chart (line colour follows theme)
  ...
  // Traffic doughnut chart (first slice follows theme)
  new Chart(document.getElementById('trafficChart').getContext('2d'), {
    ...
    data: {
      ...
      datasets: [{
        data: [35, 25, 20, 12, 8],
        backgroundColor: [THEME.primary, '#10B981', '#F59E0B', '#EF4444', '#8B5CF6'],
        borderWidth: 0
      }]
    },
    options: {
